...
def generate_ground_truth(boxes, img_size, iou_threshold=0.4):
    anchors = [(10, 13), (16, 30), (33, 23),
                (30, 61), (62, 45), (59, 119),
                (116, 90), (156, 198), (373, 326)]
    scales = [32, 16, 8]
    gt_boxes = {cls: [] for cls, _ in boxes.items()}
    gt_indices = {cls: [] for cls, _ in boxes.items()}
    grid_sizes = np.array([img_size // scales[i] for i in range(3)])
    for i in range(3):
        grid_size = grid_sizes[i]
        for gx, gy in product(range(grid_size), repeat=2):
            center = (gx + .5) * scales[i], (gy + .5) * scales[i]
            for anchor_id, anchor in enumerate(anchors[6-i*3:9-i*3]):
                box = [
                    center[0] - anchor[0] / 2,
                    center[1] - anchor[1] / 2,
                    center[0] + anchor[0] / 2,
                ...
                for cls, bboxs in boxes.items():
                    ious = np.array([_iou(box, x) for x, _ in bboxs])
                    iou_mask = ious > iou_threshold
                    if np.any(iou_mask):
                        # gt_boxes[cls] += [(np.array(box), 1.)]
                        gt_boxes[cls] += [(copy.deepcopy(x), p) for x, p in np.array(bboxs)[iou_mask]]
                        gt_indices[cls] += [
                            (np.sum((grid_sizes**2)[:i]) + gy*grid_size + gx) * 3 + anchor_id
                        ]

    return gt_boxes, gt_indices
